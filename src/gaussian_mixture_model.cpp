...
#pragma omp parallel for reduction(+ : log_likelihood)
    for (int i = 0; i < dataset_size; ++i) {
        log_likelihood += std::log(
                probability_density_function(dataset.row(i).transpose()));
    }

    return log_likelihood;
}

void GaussianMixtureModel::expectationMaximization(
...
#pragma omp parallel for
    for (int i = 0; i < dataset_size; ++i) {
        ...
        for (int j = 0; j < num_components_; ++j) {
            expectations_(i, j) =
                    prior_vec_[j] *
                    gaussian_vec_[j].evaluate_point(dataset.row(i));
...
void GaussianMixtureModel::maximization(const Eigen::MatrixXd& dataset) {
    ...
    for (int i = 0; i < num_components_; ++i) {
        ...
        Eigen::VectorXd mean = inverse_cluster_expectation *
                               expectations_.col(i).transpose() * dataset;
        Eigen::MatrixXd covariance =
                Eigen::MatrixXd::Zero(dataset.cols(), dataset.cols());
...
#pragma omp parallel
        {
            ...
            int thread_dataset_size =
                    floor(double(dataset_size) / double(num_threads));
            int thread_id = omp_get_thread_num();
            int thread_j_min = thread_dataset_size * thread_id;
            int thread_j_max;

            Eigen::MatrixXd t_covariance =
                    Eigen::MatrixXd::Zero(dataset.cols(), dataset.cols());
            ;
...
double GaussianMixtureModel::bayesianInformationCriterion(
        const Eigen::MatrixXd& dataset) {
    int dataset_size = dataset.rows();
    int point_dimensionality = dataset.cols();
    double free_parameters =
            (num_components_ - 1) * num_components_ *
            (point_dimensionality +
                    0.5 * point_dimensionality * (point_dimensionality + 1));

    double bic = -logLikelihood(dataset) +
                 0.5 * free_parameters * std::log(dataset_size);

    return bic;
}

GaussianMixtureModel GaussianMixtureModel::copy() const {
    return GaussianMixtureModel(num_components_, delta_, num_iterations_,
            prior_vec_, gaussian_vec_, expectations_);
}
...
void GaussianMixtureModel::load(Eigen::MatrixXd model) {
    ...
    for (int i = 0; i < num_components; ++i) {
        ...
        Eigen::MatrixXd covariance =
                model.block(idx + 2, 0, dimensionality, dimensionality);
