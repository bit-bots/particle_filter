...
void KMeans::forgyInitialization(const Eigen::MatrixXd& dataset) {
    ...
    for (int i = 0; i < num_clusters_; ++i) {
        ...
        } while (assigned_datapoint_idx.find(random_datapoint_idx) !=
                 assigned_datapoint_idx.end());
...
void KMeans::assign(const Eigen::MatrixXd& dataset) {
    ...
    for (int i = 0; i < dataset.rows(); ++i) {
        ...
        for (int j = 0; j < num_clusters_; ++j) {
            double distance =
                    (dataset.row(i).transpose() - means_[j]).squaredNorm();
        ...
        for (int j = 0; j < dataset.rows(); ++j) {
            if (assignments_[j] == i) {
                Eigen::VectorXd distance_vector =
                        dataset.row(j).transpose() - means_[i];
                covariances_[i] +=
                        distance_vector * distance_vector.transpose();
            }
        }
