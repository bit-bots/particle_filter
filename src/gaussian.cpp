  ...
  void Gaussian::addToEigenMatrix(Eigen::MatrixXd& matrix, float x0, float y0, float x1, float y1, int stepcount) const {
      ...
      for (int y_step = 0; y_step < stepcount; y_step++) {
          ...
          for (int x_step = 0; x_step < stepcount; x_step++) {
              ...
              // Function taken from https://en.wikipedia.org/wiki/Gaussian_function#Two-dimensional_Gaussian_function
              //matrix(y_step, x_step) += 1 * std::exp(-(std::pow(x - x_mean, 2) / covariance_(0, 0)  + 2 * (x - x_mean) * (y - y_mean) / covariance_(1, 0) + std::pow(y - y_mean, 2) / covariance_(1, 1)));
              matrix(y_step, x_step) += 1 * std::exp(-((std::pow(x - x_mean, 2) / (2 * covariance_(0, 0)))  + (std::pow(y - y_mean, 2) / (2 * covariance_(1, 1)))));
          }
      }
  }
